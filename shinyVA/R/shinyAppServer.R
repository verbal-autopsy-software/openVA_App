           ...
           setProgress(message = paste("Starting analysis of data (this may take a while)"))
            ...
            if(input$algorithm=="InSilicoVA_2012"){
                ...
                if(input$byAll){
                    ...
                    rv$fitAll <- do.call("openva::codeVA", list(data=records, model="InSilicoVA",
                ...
                if(input$bySex){
                    ...
                    if(length(male[male])>0){
                        ...
                        try(rv$fitMale <- do.call("openVA::codeVA", list(data=records[male,], model="InSilicoVA",
                    ...
                    if(length(female[female])>0){
                        ...
                        try(rv$fitFemale <- do.call("openVA::codeVA", list(data=records[female,], model="InSilicoVA",
                                                                   Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                ...
                if(input$byAge){
                    ...
                    if(length(neonate[neonate])>0){
                        ...
                        try(rv$fitNeonate <- do.call("openVA::codeVA", list(data=records[neonate,], model="InSilicoVA",
                                                                   Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                    ...
                    if(length(child[child])>0){
                        ...
                        try(rv$fitChild <- do.call("openVA::codeVA", list(data=records[child,], model="InSilicoVA",
                                                                  Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                    ...
                    if(length(adult[adult])>0){
                        ...
                        try(rv$fitAdult <- do.call("openVA::codeVA", list(data=records[adult,], model="InSilicoVA",
                                                                  Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
            ...
            if(input$algorithm=="InSilicoVA_2016"){
                ...
                if(input$byAll){
                    ...
                    okRun <- try(rv$fitAll <- insilico(records, Nsim = isolate(input$simLength), burnin = burn,
                                                       model="InSilicoVA", data.type="WHO2016", warning.write = TRUE),
                                 silent = FALSE)
                    ## rv$fitAll <- do.call("openVA::codeVA", list(data=records, model="InSilicoVA", data.type="WHO2016",
                    ##                                     Nsim=input$simLength, burnin=burn, warning.write=TRUE))
                    if(!is.null(rv$fitAll)){
                      rv$indivCODAll <- indivCOD(rv$fitAll, top=3)
                ...
                if(input$bySex){
                    ...
                    if(length(male[male])>0){
                      try(
                        ...
                        ## try(rv$fitMale <- do.call("openVA::codeVA", list(data=records[male,], model="InSilicoVA", data.type="WHO2016",
                        ##                                          Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                        if(!is.null(rv$fitMale)){
                    ...
                    if(length(female[female])>0){
                        rv$fitFemale <- insilico(records[female,], Nsim = isolate(input$simLength), burnin = burn,
                                                 model="InSilicoVA", data.type="WHO2016", warning.write = TRUE)
                        ## try(rv$fitFemale <- do.call("openVA::codeVA", list(data=records[female,], model="InSilicoVA", data.type="WHO2016",
                        ##                                            Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)
                                                    ## ))
                        if(!is.null(rv$fitFemale)){
                ...
                if(input$byAge){
                    ...
                    if(length(neonate[neonate])>0){
                        rv$fitNeonate <- insilico(records[neonate,], Nsim = isolate(input$simLength), burnin = burn,
                                                  model="InSilicoVA", data.type="WHO2016", warning.write = TRUE)
                        ## try(rv$fitNeonate <- do.call("openVA::codeVA", list(data=records[neonate,], model="InSilicoVA", data.type="WHO2016",
                        ##                                            Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                        if(!is.null(rv$fitNeonate)){
                    ...
                    if(length(child[child])>0){
                        rv$fitChild <- insilico(records[child,], Nsim = isolate(input$simLength), burnin = burn,
                                                model="InSilicoVA", data.type="WHO2016", warning.write = TRUE)
                        ## try(rv$fitChild <- do.call("openVA::codeVA", list(data=records[child,], model="InSilicoVA", data.type="WHO2016",
                        ##                                           Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                        if(!is.null(rv$fitChild)){
                    ...
                    if(length(adult[adult])>0){
                        rv$fitAdult <- insilico(records[adult,], Nsim = isolate(input$simLength), burnin = burn,
                                                model="InSilicoVA", data.type="WHO2016", warning.write = TRUE)
                        ## try(rv$fitAdult <- do.call("openVA::codeVA", list(data=records[adult,], model="InSilicoVA", data.type="WHO2016",
                        ##                                           Nsim=isolate(input$simLength), burnin=burn, warning.write=TRUE)))
                        if(!is.null(rv$fitAdult)){
            ...
            if(input$algorithm=="InterVA4"){
                ...
                if(input$byAll){
                    incProgress(.01, detail=paste("Analysis with all cases"))
                    rv$fitAll     <- InterVA4::InterVA(Input=records, HIV=input$HIV, Malaria=input$Malaria)
                ...
                if(input$bySex){
                    ...
                    if(length(male[male])==0) rv$male <- NULL
                    if(length(male[male])>0){
                        rv$fitMale     <- InterVA4::InterVA(Input=records[male,], HIV=input$HIV, Malaria=input$Malaria)
                    ...
                    if(length(female[female])>0){
                        ## rv$female <- TRUE
                        rv$fitFemale     <- InterVA4::InterVA(Input=records[female,], HIV=input$HIV, Malaria=input$Malaria)
                ...
                if(input$byAge){
                    ...
                    if(length(neonate[neonate])>0){
                        ## rv$neonate <- TRUE
                        rv$fitNeonate     <- InterVA4::InterVA(Input=records[neonate,], HIV=input$HIV, Malaria=input$Malaria)
                    ...
                    if(length(child[child])>0){
                        ## rv$child <- TRUE
                        rv$fitChild     <- InterVA4::InterVA(Input=records[child,], HIV=input$HIV, Malaria=input$Malaria)
                    ...
                    if(length(adult[adult])>0){
                        ## rv$adult <- TRUE
                        rv$fitAdult     <- InterVA4::InterVA(Input=records[adult,], HIV=input$HIV, Malaria=input$Malaria)
            ...
            if(input$algorithm=="InterVA5"){
                ...
                if(input$byAll){
                    ...
                    rv$fitAll     <- InterVA5::InterVA5(Input=records, HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                        filename="VA_result")
                    rv$resultsAll <- read.csv("VA_result.csv")
                    ...
                    if(file.exists(plotName)) file.remove(plotName)
                    ## pdf(plotName);CSMF(rv$fitAll, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001);dev.off()
                    pdf(plotName);InterVA5::CSMF5(rv$fitAll, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                    output$downloadPlot1 <- downloadHandler(
                        #filename = plotName,
                ...
                if(input$bySex){
                    ...
                    if(length(male[male])==0) rv$male <- NULL
                    if(length(male[male])>0){
                        rv$fitMale     <- InterVA5::InterVA5(Input=records[male,], HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                            filename="VA_result")
                        rv$resultsMale <- read.csv("VA_result.csv")
                        ...
                        if(file.exists(plotName)) file.remove(plotName)
                        ## pdf(plotName);CSMF(rv$fitMale, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001);dev.off()
                        pdf(plotName);InterVA5::CSMF5(rv$fitMale, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                        output$downloadPlot2 <- downloadHandler(
                            #filename = plotName,
                    ...
                    if(length(female[female])>0){
                        ## rv$female <- TRUE
                        rv$fitFemale     <- InterVA5::InterVA5(Input=records[female,], HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                            filename="VA_result")
                        rv$resultsFemale <- read.csv("VA_result.csv")
                        ...
                        if(file.exists(plotName)) file.remove(plotName)
                        ## pdf(plotName);CSMF(rv$fitFemale, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001);dev.off()
                        pdf(plotName);InterVA5::CSMF5(rv$fitFemale, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                        output$downloadPlot3 <- downloadHandler(
                ...
                if(input$byAge){
                    ...
                    if(length(neonate[neonate])>0){
                        ## rv$neonate <- TRUE
                        rv$fitNeonate     <- InterVA5::InterVA5(Input=records[neonate,], HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                            filename="VA_result")
                        rv$resultsNeonate <- read.csv("VA_result.csv")
                        ...
                        if(file.exists(plotName)) file.remove(plotName)
                        ## pdf(plotName);CSMF(rv$fitNeonate, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001);dev.off()
                        pdf(plotName);InterVA5::CSMF5(rv$fitNeonate, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                        output$downloadPlot4 <- downloadHandler(
                    ...
                    if(length(child[child])>0){
                        ## rv$child <- TRUE
                        rv$fitChild     <- InterVA5::InterVA5(Input=records[child,], HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                            filename="VA_result")
                        rv$resultsChild <- read.csv("VA_result.csv")
                        ...
                        if(file.exists(plotName)) file.remove(plotName)
                        ## pdf(plotName);CSMF(rv$fitChild, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001);dev.off()
                        pdf(plotName);InterVA5::CSMF5(rv$fitChild, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                        output$downloadPlot5 <- downloadHandler(
                    ...
                    if(length(adult[adult])>0){
                        ## rv$adult <- TRUE
                        rv$fitAdult     <- InterVA5::InterVA5(Input=records[adult,], HIV=input$HIV, Malaria=input$Malaria, directory=getwd(),
                            filename="VA_result")
                        rv$resultsAdult <- read.csv("VA_result.csv")
                        ...
                        if(file.exists(plotName)) file.remove(plotName)
                        ## pdf(plotName); CSMF(rv$fitAdult, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001); dev.off()
                        pdf(plotName);InterVA5::CSMF5(rv$fitAdult, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001);dev.off()
                        output$downloadPlot6 <- downloadHandler(
...
#      )
        if(!is.null(rv$fitAll)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitAll$HIV)){
                ## CSMF(rv$fitAll, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitAll, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
    ...
    output$plotMale <- renderPlot({
        if(!is.null(rv$fitMale)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitMale$HIV)){
                ## CSMF(rv$fitMale, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitMale, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
    ...
    output$plotFemale <- renderPlot({
        if(!is.null(rv$fitFemale)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitFemale$HIV)){
                ## CSMF(rv$fitFemale, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitFemale, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
    ...
    output$plotNeonate <- renderPlot({
        if(!is.null(rv$fitNeonate)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitNeonate$HIV)){
                ## CSMF(rv$fitNeonate, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitNeonate,, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
    ...
    output$plotChild <- renderPlot({
        if(!is.null(rv$fitChild)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitNeonate$HIV)){
                ## CSMF(rv$fitChild, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitChild, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
            if(input$algorithm=="InSilicoVA_2012" & is.null(rv$fitNeonate$HIV)){
                ## indivplot(rv$agg.csmfChild, top=20, title="Aggregated COD distribution")
    ...
    output$plotAdult <- renderPlot({
        if(!is.null(rv$fitAdult)){
            ...
            if(input$algorithm=="InterVA5" & !is.null(rv$fitAdult$HIV)){
                ## CSMF(rv$fitAdult, top.plot=input$topDeaths, InterVA.rule=TRUE, min.prob=.001)
                InterVA5::CSMF5(rv$fitAdult, top.plot=input$topDeaths, InterVA.rule = TRUE, min.prob=.00001)
            }
